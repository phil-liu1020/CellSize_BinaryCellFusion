function f = Survival_function_different(mgenotypes,alphagenotypes,x,S,beta,Cz,Cp)
   ...
   wk=0; 
        ...
        Mk = Mv(any(Mv==k,2),:);        % Mk outputs an array showing only the columns of x (which ODE) that contains genotype k cells.     
           for ii=1:length(Mk(:,1))     % This for loop with index ii calculates the absolute fitness of the k-th genotype.
              ...
              wk=wk+x(length(x(:,1)),Mk(ii,1))*(1 - exp(-beta*( mgenotypes(Mk(ii,2)+1) + mgenotypes(Mk(ii,3)+1) ) ))*(1-Cp);    % any rows with k in their 2nd or 3rd columns. the +1 is here because the first entry of mgenotypes is set to 0 for a purpose. If an entry in Mk is 0, then it adds mgenotypes(1).  
              else
              wk=wk+x(length(x(:,1)),Mk(ii,1))*(1 - exp(-beta*( mgenotypes(Mk(ii,2)+1) + mgenotypes(Mk(ii,3)+1) ) ))*(1-Cz);
              end
